...
import { Search, MapPin, Star, ShieldCheck, Users, BedDouble, Calendar, Tag, ArrowRight, Zap, ChevronRight } from "lucide-react";
import { useListApartments, useListEquipment } from "@workspace/api-client-react";
import { formatCurrency } from "@/lib/utils";

export function PowderDayPage() {
  ...
  return (
    <div className="min-h-screen bg-white text-[#111827]" style={{ fontFamily: "'Barlow', sans-serif" }}>
      <style>{`
          ...
          <div className="max-w-4xl mx-auto bg-white rounded-xl shadow-2xl p-4 md:p-6 border-t-8 border-[#FF5C2B]">
            ...
            {searchTab === "apartments" ? (
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <button className="bg-[#FF5C2B] hover:bg-[#E0481D] text-white rounded-lg p-4 flex items-center justify-center font-syne font-bold uppercase tracking-widest text-lg gap-2 shadow-lg shadow-[#FF5C2B]/30 hover:shadow-xl transition-all active:scale-95">
              ...
              <div className="flex flex-col md:flex-row gap-4">
                ...
                <button className="bg-[#FF5C2B] hover:bg-[#E0481D] text-white rounded-lg px-8 py-4 flex items-center justify-center font-syne font-bold uppercase tracking-widest text-lg gap-2 shadow-lg shadow-[#FF5C2B]/30 transition-all active:scale-95">
                  Buscar Equipo
                </button>
              </div>
            )}
      ...
      <section className="py-24 bg-white">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <div className="flex justify-between items-end mb-12 border-b-4 border-[#111827] pb-4">
            <div>
              <h2 className="font-syne text-4xl md:text-5xl font-extrabold text-[#111827] uppercase tracking-tight">
                Departamentos <span className="text-[#FF5C2B]">Verificados</span>
              </h2>
              ...
              <div key={apt.id} className="group relative bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2">
                ...
                <div className="relative aspect-[4/3] overflow-hidden">
                  ...
                  <div className="absolute bottom-4 right-4 bg-[#111827] text-white px-3 py-1.5 rounded-lg flex items-center gap-1.5 shadow-lg">
                    ...
                    <span className="text-sm font-bold">{Number(apt.rating).toFixed(1)}</span>
                  </div>
                </div>
      ...
      <section className="py-24 bg-[#F0F9FF]">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <div className="flex justify-between items-end mb-12 border-b-4 border-[#0EA5E9] pb-4">
            <div>
              <h2 className="font-syne text-4xl md:text-5xl font-extrabold text-[#111827] uppercase tracking-tight">
                Marketplace de <span className="text-[#0EA5E9]">Equipos</span>
              </h2>
      ...
      <section className="py-24 bg-[#FF5C2B] text-white relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-8 relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
            {[
              { bg: "bg-[#111827]", icon: <Zap className="w-12 h-12 text-[#FF5C2B]" />, title: "Rápido y Directo", textColor: "text-white", desc: "Sin intermediarios innecesarios. Conecta directamente con dueños y vendedores al instante." },
              { bg: "bg-white", icon: <ShieldCheck className="w-12 h-12 text-[#FF5C2B]" />, title: "100% Seguro", textColor: "text-[#111827]", desc: "Propiedades verificadas en terreno y equipos revisados en nuestra bodega antes de la entrega." },
              { bg: "bg-[#0EA5E9]", icon: <Tag className="w-12 h-12 text-white" />, title: "Mejores Precios", textColor: "text-white", desc: "Comisión transparente y justa. Paga menos que en otras plataformas y disfruta más la montaña." },
            ].map((item) => (
              <div key={item.title} className="flex flex-col items-center text-center group">
                <div className={`w-24 h-24 rounded-2xl ${item.bg} flex items-center justify-center mb-8 transform -skew-x-6 group-hover:scale-110 transition-transform shadow-xl`}>
                  {item.icon}
                </div>
                <h3 className={`font-syne text-3xl font-bold mb-4 uppercase ${item.textColor}`}>{item.title}</h3>
                <p className="font-barlow text-xl font-medium text-white/90">{item.desc}</p>
              </div>
            ))}
          </div>
        </div>
